...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Eonarc_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Eonarc_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Eonarc_RIG", 20, None).await?;
...
Created by [Eonarc](https://x.com/Eonarc_RIG)
