...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Mytrix_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Mytrix_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Mytrix_RIG", 20, None).await?;
...
Created by [Mytrix](https://x.com/Mytrix_RIG)
