...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Zyler_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Zyler_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Zyler_RIG", 20, None).await?;
...
Created by [Zyler](https://x.com/Zyler_RIG)
