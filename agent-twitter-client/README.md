...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Althar_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Althar_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Althar_RIG", 20, None).await?;
...
Created by [Althar](https://x.com/Althar_RIG)
