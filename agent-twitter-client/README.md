...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Cuylur_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Cuylur_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Cuylur_RIG", 20, None).await?;
...
Created by [Cuylur](https://x.com/Cuylur_RIG)
