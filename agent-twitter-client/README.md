...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Axivest_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Axivest_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Axivest_RIG", 20, None).await?;
...
Created by [Axivest](https://x.com/Axivest_RIG)
