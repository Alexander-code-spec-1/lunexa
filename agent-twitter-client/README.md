...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Avenia_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Avenia_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Avenia_RIG", 20, None).await?;
...
Created by [Avenia](https://x.com/Avenia_RIG)
