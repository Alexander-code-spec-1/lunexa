...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("Kypher_RIG").await?;
    
    // Get user profile
    let profile = scraper.get_profile("Kypher_RIG").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("Kypher_RIG", 20, None).await?;
...
Created by [Kypher](https://x.com/Kypher_RIG)
